...
def get_architectural_change(first_arch, second_arch):
    """Returns the list of changes between two architectures"""
    return get_architectural_change_by_flow(first_arch, second_arch)


def get_architectural_change_by_flow(first_arch, second_arch):
    changes = []
    for (clusterA, clusterB) in get_matched_by_flow_clusters(first_arch, second_arch):
        removed_classes = get_removed_classes(clusterA, clusterB)
        added_classes = get_added_classes(clusterA, clusterB)
...
def get_removed_classes(first_cluster, second_cluster, removed = True):
    elements = []
...
def get_added_classes(first_cluster, second_cluster):
    return get_removed_classes(second_cluster, first_cluster, False)
