...
var arrow_x = [-300, 200];
var arrow_y = [50, -10];
var layer_history_x = ["-=150", "+=100"];
var node_history_x = ["-=300", "+=200"];
var layer_history_y = ["+=25", "-=5"];
var node_history_y = ["+=50", "-=10"];

var reverse_layer_history_x = ["+=150", "-=100"];
var reverse_node_history_x = ["+=300", "-=200"];
var reverse_layer_history_y = ["-=25", "+=5"];
var reverse_node_history_y = ["-=50", "+=10"];
...
var input_file_name = "data/android/" + params.recovery + "/" + params.ver1 + "_" + params.ver2 + "_" + params.layer + "/" + params.ver1 + "_" + params.ver2 + "_" + params.target_sub + "_processed_archs.json"
...
function renderGraph() {
  ...
  $("#ver1_btn").text($("#filename1").val());
  $("#ver2_btn").text($("#filename2").val());
      ...
      for (i=0; i<root.children[v].clusters.length; i++){
        ...
        $("#cluster_list"+v).append(
          "<li class='color_list'>"  
            ...
            + "<div class='list_index' style='display: none;'> <a href='" + "http://localhost:8888/EVA/Front-end/index_single.html?&recovery=" + params.recovery + "&ver1=" + params.ver1 + "&ver2=" + params.ver2 + "&single=" + root.version_list[v] + "&layer=" + params.layer + "&target_sub=" + root.children[v].clusters[i] + "'>" + root.children[v].clusters[i] + "</a></div>"
        + "</li>\n");
      }
    }
    ...
    ver_nodes[i].append("circle")
        ...
        .on("mouseover", function(d) {
          type_dict = ['Enhancement', 'Bug', 'Documentation', 'Performance', 'Test', 'Feature', '*Enhancement', '*Bug', '*Documentation', '*Performance', '*Test', '*Feature']
            var issue_desc = "";
            if ('issue' in d.data) {
              $("#issue_box").text(d.data.issue);
            }
            if ('description' in d.data) {
              $("#desc_box").text(d.data.description);
            }
            else {
            ...
            if ('label' in d.data) {
              $("#label_box").text(type_dict[parseInt(d.data.label)])
            }
            ...
            if ($("#show_history").is(':checked')) {
              ...
              link.style("visibility", function(d){
                ...
                // if (d.source.name == cur_node.data.name && cur_node.data.version == start_idx ){
                  return "visible";
                }
                else
                  return "hidden";
