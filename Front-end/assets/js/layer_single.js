...
function renderGraph() {
  ...
  d3.json(input_file_name, function(error, root) {
    ...
    var print_version = params.single
    var version_id = "ver" + print_version.replace(/\./i, '').replace(/\s/g, '');
    $("#partial_title").html(print_version + " - " + params.target_sub)
  ...
  showSingleVersion(version_list[0], num_ver);
    ...
    ver_nodes[i].append("circle")
        ...
        .on("mouseover", function(d) {
          ...
          // if (d.version == cur_idx.toString()) {
            var issue_desc = "";
            if ('issue' in d.data) {
              // var issue_link = '<a href="' + d.data.url + '">' + d.data.issue + "</a>"
              // $("#issue_box").text(issue_link);
